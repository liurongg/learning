...
<body class="gray-bg">
        ...
        <div class="row">
			  <div class="col-lg-2">
				<div class="input-group">
			      ...
			      <input type="text" name="schoolname" class="form-control" id="txt_search_username" >
				</div>
			  </div>
			  <div class="col-lg-2">
				<div class="input-group">
					...
					<form id="form_user" method="post" action="reserveUser.htm">
						...
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							...
							<tr>
								<td>所在城市：</td>
								<td colspan="4">
									<select class="form-control" name="areaid"   id = "areaid" onaria-required="true" required>
							...
							<tr>
								<td>是否运营：</td>
								<td>
									<input type="radio" id="status" name="status" value="是"  aria-required="true" required/>是
									<input type="radio" id="status" name="status" value="否"  aria-required="true" required/>否
								</td>
								<td>&nbsp;&nbsp;</td>								
							</tr>
						</table>
	...
	<!--删除对话框 -->
	<div class="modal fade" id="modal_export" role="dialog" aria-labelledby="modal_user_del" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					 <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
				...
				<div class="modal-footer">
				<button type="button" class="btn btn-danger"  id="export_btn">确定</button>
				<button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
			</div>
			</div>
		</div>
	...
	var TableInit = function () {
	    ...
	    oTableInit.queryParams = function (params) {
	        var temp = {//这里的键的名字和控制器的变量名必须一致，这边改动，控制器也需要改成一样的
	            ...
	            schoolname: $("#txt_search_username").val(),
	...
	function initEditUser(getSelection){
		...
		fun1(getSelection.status);
		getcheng(getSelection.areaid);
	}
	 ...
	 window.onload=$(function(){
			$.ajax({
			    ...
			    success:function(res){
			    	if(res.success){
		    			var cheng=res.list;	
		    			cheng.forEach(function(item){
		    				var chengshiDm="<option value='"+item.id+"'>"+item.areaname+"</option>";
		    				$("#areaid").append(chengshiDm);
		    			});
		    		}else{
		    			$("#select_message").text(res.errorMsg);
		    			$("#alertmod_table_user_mod").show();
		...
		function fun1(status) {
			if(status=="是"){
				$("#status[value='是']").attr("checked",true);
			}else{
				$("#status[value='否']").attr("checked",true);
			}
		}
		function  getcheng(areaid){
			$("#areaid").empty();
			$.ajax({
			    url:"getcheng.htm",
			    dataType:"json",
			    ...
			    success:function(res){
			    	if(res.success){
		    			...
		    			cheng.forEach(function(item){
		    				var chengshiDm="<option value='"+item.id+"'>"+item.areaname+"</option>";
		    				if(areaid==item.id){
		    					chengshiDm="<option selected='selected' value='"+item.id+"'>"+item.areaname+"</option>";
		    				}
		    				$("#areaid").append(chengshiDm);
		    			});
		    		}else{
